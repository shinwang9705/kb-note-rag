...
- 提供以对话为核心的多轮问答工作区，支持历史会话、流式输出、停止生成、知识范围、回答策略和高级生成参数。
- 依据检索结果生成带 `[n]` 引用的回答，可从答案直接定位原文。
- 支持 DeepSeek、通义千问、豆包和智谱等 OpenAI 兼容模型端点；API Key 由用户在本地配置并加密保存。
- Embedding 与 Rerank 模型可直接在设置页配置、测试和启停，兼容 OpenAI Embedding 接口及 DashScope 原生端点。
- 分块规则支持结构化分块、滑动窗口、片段长度、重叠长度、句子边界和章节路径保留。
- 向量或重排序服务不可用时可降级到关键词检索；检索依据不足时拒绝无依据生成。
- 提供知识库管理、标签与收藏、检索历史、用量统计、问答质量驾驶舱和管理员用户管理。
- 内置持久化后台任务队列，支持任务认领、租约续期、取消与失败恢复；索引代次可追踪当前版本和重建状态。
- 内置登录失败锁定、JWT 注销黑名单、模型端点来源限制、上传配额和多用户权限校验。
- 支持浅色、深色和高对比度主题，并针对桌面端与移动端进行响应式适配。
- 前端页面、图表和动画依赖按需拆包，降低首屏加载体积。
...
所有私密配置只应写入本地 `.env` 或通过设置页保存，切勿提交真实密钥。`.env.example` 的密钥和初始管理员密码均保持为空。
...
将真实 `LLM_API_KEY` 填入本地 `.env`，或登录后在设置页保存个人供应商凭据。Embedding、Rerank、分块规则、召回数量、置信度阈值和生成参数也可在设置页管理。系统支持的关键配置包括：
...
| `PROVIDER_ALLOWED_ORIGINS` | 管理员额外授权的可信模型网关 origin，多个地址用逗号分隔 |
...
项目包含认证、权限隔离、文档入库、检索、RAG、会话、后台任务、索引版本、流式协议、供应商路由、备份和统计等 259 项测试。提交前建议执行：
...
当前仍适合继续增强的方向包括固定检索评测集、多节点任务调度、全局/IP/成本限流、大会话虚拟滚动，以及大规模知识库的增量向量重建。
